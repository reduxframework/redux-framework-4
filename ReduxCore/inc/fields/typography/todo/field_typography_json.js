...
(function( $ ) {
	...
	var default_params = {
		width: 'resolve', triggerChange: true, allowClear: true
	};
	...
	redux.field_objects.typography.init = function( selector, skipCheck ) {
		...
		$( selector ).each( function() {
			...
			el.each( function() {
				...
				$( this ).find( '.redux-typography-container' ).each( function() {
				...
				$( this ).find( '.redux-typography' ).on( 'change', function() {
					redux.field_objects.typography.select( $( this ) ); //.parents('.redux-container-typography:first'));
				} );

				//init when value is changed
				$( this ).find( '.redux-typography-size, .redux-typography-height, .redux-typography-word, .redux-typography-letter, .redux-typography-align, .redux-typography-transform, .redux-typography-font-variant, .redux-typography-decoration' ).keyup( function() {
					redux.field_objects.typography.select( $( this ).parents( '.redux-container-typography:first' ) );
				} );
				...
				$( this ).find( ".redux-typography-family" ).select2( {
					...
					initSelection: function( element, callback ) {
						...
						callback( data );
					}, allowClear: fontClear, // when one clicks on the font-family select box
					...
					if ( isUserFonts == 1 ) {
						...
						for ( var key in redux.typography[thisID] ) {
							...
							for ( var prop in obj ) {
								if ( obj.hasOwnProperty( prop ) ) {
									fontKids.push( {
										id: prop, text: prop, 'data-google': 'false'
									} );
								}
							}
						}
						...
						// <optgroup>
						var fontData = {
							text: 'Standard Fonts', children: fontKids
						};
	...
	redux.field_objects.typography.change = function( container ) {
		var $data = {};
		jQuery( container ).find( '.redux-typography-input' ).each( function() {
	...
	redux.field_objects.typography.select = function( selector, skipCheck ) {
		...
		if ( google === true && (family in redux.fonts.google) ) {
			...
			details = {
				'400': 'Normal 400', '700': 'Bold 700', '400italic': 'Normal 400 Italic', '700italic': 'Bold 700 Italic'
			};
		}
