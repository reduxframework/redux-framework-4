				...
				el.each(
					function () {
						...
						$(this).find('#redux-export-code-dl').click(function (e) {
							e.preventDefault();
							if (!!window.onbeforeunload && confirm('Your panel has unchanged values, would you like to save them now?')) {
						...
						$(this).find('#redux-export-code-copy').click(
							function (e) {
								...
								setTimeout(() => {
									...
									$.get(`${ajaxurl}?download=0&action=redux_download_options-${redux.optName.args.opt_name}&secret=${secret}`, function (data) {
