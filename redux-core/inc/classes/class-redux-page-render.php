	...
	class Redux_Page_Render extends Redux_Class {
		...
		public function load_page() {
			...
			if ( true === $this->show_hints ) {
				...
				$msg = sprintf( esc_html__( 'Hints are tooltips that popup when %1$s the hint icon, offering addition information about the field in which they appear.  They can be %2$s by using the link below.', 'redux-framework' ), $event, mb_strtolower( $s ) ) . '<br/><br/>' . $url;
		...
		private function submenu( $core ) {
			...
			$page_parent = mb_strtolower( $page_parent );
